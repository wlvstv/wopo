...
async function getCasterInfo(channel, user, message) {
  if(user.mod || user.username.toString().toLowerCase() == "decoydix"){
    ...
    if (messageArray.length <= 1) {
      ...
      wolvesChat.say(
        channel,
        `Check out ${casterInfo.name} over at ${casterInfo.URL} they are a great friend of the channel and we encourage you to drop them a follow! Last playing ${casterInfo.lastGame}`
      );
    }
  }
}
