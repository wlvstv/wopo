...
/*
 * !clip: Broadcasts a url of a clip within the past 30 seconds
 */
async function clipCommand(channel) {
  if (util.isLive(config.channelID)) {
    api.createClipURL().then(clipURL => wolvesChat.say(channel, clipURL))
  } else {
    wolvesChat.say(channel, 'Wolvesatmydoor is currently offline, check @wlvsatmydoor on twitter for updates on stream times!')
  }
...
async function nowPlayingCommand(channel, user, message) {
  spotify.nowPlaying().then((songInfo) => {
    const {name, artist, url} = songInfo
    wolvesChat.say(channel, `Currently Playing: ${name} by ${artist} ${url}`)
  }).catch(() => {
    wolvesChat.say(channel, 'Woops...Spotify isn\'t connected right.')
  })
}
