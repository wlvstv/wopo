...
function updateCommand(channel, user, message) {
  ...
  if (util.isMod(user)) {
    let command = message.split(' ')[1];
    if (!command.toString().startsWith('!')) return;
    let response = message.split(' ').splice(2).join(' ');
    db.updateCommand(
      command,
      true,
      response,
  ...
  if (util.isMod(user)) {
    if (params.slice(1)[0] === 'stop' && _cronSchedule) {
      ...
      if (_doesCommandExist && !_cronSchedule) {
        ...
        wolvesChat.say(
          channel,
          `That's awkward...I don't know the ${_command} command.`
        );
...
function hoopsCommand(channel, user, message) {
  const params = message.split(' ');
  if (params.length > 1) {
    if (user.mod || user.username == 'wolvesatmydoor') {
      db.addHoop(params[1]);
    }
  } else {
    db.getHoops(function (hoops) {
      let alltimeMade = parseInt(hoops[0].total);
      let alltimeMissed = parseInt(hoops[1].total);
      let today = 'TODAY: 0/0 (0%)';
      if (hoops.length > 2) {
        if (hoops.length === 3) {
          let ttl = parseInt(hoops[2].total);
          if (hoops[2].results === 'TODAY MADE') {
            alltimeMade = alltimeMade + ttl;
            today = `TODAY: ${ttl} / ${ttl} (100%)`;
          } else {
            alltimeMissed = alltimeMissed + ttl;
            today = `TODAY: ${ttl} / ${ttl} (0%)`;
          }
        } else {
          let todayMade = parseInt(hoops[2].total);
          let todayMissed = parseInt(hoops[3].total);
          alltimeMade = alltimeMade + todayMade;
          alltimeMissed = alltimeMissed + todayMissed;
          today = `TODAY: ${todayMade}/${todayMade + todayMissed} (${Math.round(
            (todayMade / (todayMade + todayMissed)) * 100
          )}%)`;
        }
      }
      let alltime = `ALL TIME: ${alltimeMade}/${
        alltimeMade + alltimeMissed
      } (${Math.round((alltimeMade / (alltimeMade + alltimeMissed)) * 100)}%)`;
      wolvesChat.say(channel, `${today} | ${alltime}`);
    });
  }
}
