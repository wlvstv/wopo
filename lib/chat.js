...
async function clipCommand(channel) {
  try {
    const _isLive = await util.isLive(config.channelID)
    if (_isLive) {
      const _clipURL = await api.createClipURL()
      wolvesChat.say(channel, _clipURL)
    } else {
      wolvesChat.say(channel, 'Wolvesatmydoor is currently offline, check @wlvsatmydoor on twitter for updates on stream times!')
    }
  } catch(error) {
    wolvesChat.say(channel, 'Sorry, I can\'t do that right now.')
  }
}
/*
