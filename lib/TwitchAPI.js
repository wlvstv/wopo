...
exports.channelInfo = async function(channelName) {
    let channelIDEP = `https://api.twitch.tv/helix/users?login=${channelName}`;
    let channelIDData = await openTwitch(channelIDEP, "GET");
    let channelID = channelIDData.data[0].id;
    let endpoint = `https://api.twitch.tv/kraken/channels/${channelID}`;   
    let channelData = await openTwitch(endpoint,'OGET');
  ...
  async function openTwitch(endpoint, _method) {
    ...
    let headers = {'Client-ID': botConfig.twitchCID};
    if(_method = "OGET"){
      headers = {'Client-ID': botConfig.twitchCID, 'Accept':'application/vnd.twitchtv.v5+json'};
      _method = "GET";
    }
    const resp = await fetch(endpoint, {method:_method, headers})
      .then(res => {
        if (res.status >= 400) {
