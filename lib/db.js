...
exports.addHoop=function(result, callback){
  db
  .none(
...
exports.getHoops=function(callback){ 
  return new Promise((resolve, reject) => {
  db
  .any(
    'SELECT result, count(*) as total FROM public.hoops GROUP BY result ORDER BY result '
  )
  .then((hoop) => {
    callback(hoop)
  })
