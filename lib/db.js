...
exports.addHoop = (result, callback) => {
  db.none(
    'INSERT INTO public.hoops(result, "timestamp") VALUES ($1,current_timestamp);',
...
exports.getHoops = (callback) => {
  db.any(
    `SELECT RESULTS, COUNT(*) AS TOTAL FROM (SELECT "timestamp", result, CASE WHEN "timestamp" >  CURRENT_DATE AND result = 'Y' THEN 'TODAY MADE' WHEN "timestamp" > CURRENT_DATE and result = 'N' THEN 'TODAY MISSED' WHEN result = 'Y' THEN 'ALL TIME MADE' WHEN result = 'N' THEN 'ALL TIME MISSED' END AS RESULTS FROM public.hoops) as rsx GROUP BY RESULTS ORDER BY RESULTS`
  )
    .then((hoop) => {
      callback(hoop);
    })
